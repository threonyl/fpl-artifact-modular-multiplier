r"""
Python runner for the modacc cocotb testbench.

Usage
-----
# Run with default simulator (Icarus Verilog):
    python run_modacc.py

# Choose a different simulator:
    SIM=questa python run_modacc.py

# Enable waveform dumping:
    WAVES=1 python run_modacc.py

# Run a single test by name:
    COCOTB_TESTCASE=test_corner_cases python run_modacc.py

# Enable fixed-modulus mode with a specific Q_VALUE (hex accepted):
    FIXED_Q=1 Q_VALUE=0xFFFFFFFD python run_modacc.py

# Enable the split-adder pipeline register in the addtree:
    AT_FF_ADD=1 python run_modacc.py

# Via pytest (the test_ prefix makes it auto-discoverable):
    pytest run_modacc.py
    pytest run_modacc.py -v -s          # verbose, print logs
    pytest run_modacc.py -k corner      # filter by name
...
def _collect_parameters() -> dict:
    ...
    Supported environment variables (all optional):
        LOGQ, NUM_INPUTS, REG_IN, REG_OUT, REG_ADD, CONC_ADDSUB,
        USE_ADDTREE, MAX_COND_SUB,
        AT_REG_PERIOD, AT_REG_IN, AT_REG_OUT, AT_USE_CSA,
        CS_REG_PERIOD, CS_REG_OUT, AT_FF_ADD,
        FIXED_Q, Q_VALUE
...
def test_modacc():
    """
    Build and run all modacc cocotb tests.

    The `test_` prefix means pytest discovers and executes this function
    automatically when you run `pytest run_modacc.py`.
    """
    ...
    runner.build(
        ...
        sources=[str(ts_file), str(RTL_DIR / "csa_2_pkg.sv"), str(RTL_DIR / "csa_2.sv"), str(RTL_DIR / "addtree_pkg.sv"), str(RTL_DIR / "addtree.sv"), str(RTL_DIR / "modadd_pkg.sv"), str(RTL_DIR / "modadd.sv"), str(RTL_DIR / "modacc_pkg.sv"), str(RTL_DIR / "modacc.sv"), ],
