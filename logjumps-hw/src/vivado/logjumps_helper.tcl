# --------------------------------------------------------------
# logjumps_helper.tcl, PRE_HOOK for logjumps builds
#
...
#   ./build.sh impl_logjumps.f PRE_HOOK=logjumps_helper.tcl \
...
if {$logq_prime ne "" && $q_eff_val ne ""} {
    puts "ERROR: logjumps_helper.tcl: LOGQ_PRIME and Q_EFF_VAL are mutually exclusive."
    exit 1
}
if {$logq_prime eq "" && $q_eff_val eq ""} {
    puts "ERROR: logjumps_helper.tcl requires either LOGQ_PRIME=<bits> or Q_EFF_VAL=<hex>."
    exit 1
}
...
if {$cond_sub_opt ne "" && $cond_sub_opt} {
    ...
    if {$_mcs_existing ne ""} {
        puts "ERROR: logjumps_helper.tcl: COND_SUB_OPT and ACC_MAX_COND_SUB are mutually exclusive."
        exit 1
    }
